...
describe('FileController test', function() {
    ...
    describe('move_file test', function(){
      it("should call MoveFile successfully" , function(){
        var remoteFolder = "TestData/In";
        ...
        var request = new model.MoveFileRequest();
        request.srcPath =  remoteFolder + "/" + remoteName;
        request.destPath =  "OutResult/" + remoteName;
        request.srcStorageName =  "";
        request.destStorageName =  "";
        request.versionId =  "";
        return cellsApi.moveFile(request).then((result) => {
            expect(result.response.statusCode).to.equal(200);
        });
      });
    }); 
    describe('delete_file test', function(){
      it("should call DeleteFile successfully" , function(){
        var remoteFolder = "TestData/In";
        ...
        var request = new model.DeleteFileRequest();
        request.path =  remoteFolder + "/" + remoteName;
        request.storageName =  "";
        request.versionId =  "";
        return cellsApi.deleteFile(request).then((result) => {
            expect(result.response.statusCode).to.equal(200);
        });
      });
    }); 
